...
def getLLMResponse(query,age_option,tasktype_option,numberOfWords):
    ...
    prefix = """You are a {template_ageoption}, and {template_tasktype_option} and write the response less than {numberOfWords} words: 
    ...
    new_prompt_template = FewShotPromptTemplate(
        ...
        prefix=prefix,
        suffix=suffix,
        input_variables=["template_userInput","template_ageoption","numberOfWords","template_tasktype_option"],
        example_separator="\n"
    )

    prompt=new_prompt_template.format(template_userInput=query,template_ageoption=age_option,numberOfWords=str(numberOfWords),template_tasktype_option=tasktype_option)
    print(prompt)
    response=llm(prompt)
    print(response)
...
st.header("Hey, I'm GeneratiVerse.ai Marketing Content Generation Bot. \n How can I help you?")
...
numberOfWords= st.slider('Words limit (maximum words it can use)', 1, 50, 10)
...
st.image("Generativerse.png")

st.text("Built by Dileep Bondla @ GeneeratiVerse.ai, Powered by OpenAI")
